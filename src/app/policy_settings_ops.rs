//! Asynchronous Xray `policy` top-level settings modification orchestration (Roadmap §2.1:49).
//!
//! Mirrors [`super::dns_settings_ops`] — same validate → mutate → conflict-check → backup →
//! write pipeline, just targeting `policy` instead of `dns`.
...
use crate::xray::{
    ...
    UpdatePolicySettingsRequest, update_policy_settings,
};

/// Outcome delivered from the policy-settings worker thread.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PolicySettingsMutationOutcome {
    /// Success payload or classified error.
    pub result: Result<PolicySettingsMutationSuccess, ConfigModifyError>,
}

/// Successful remote write of updated policy settings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PolicySettingsMutationSuccess {
    /// Editable model after the in-memory mutation (already applied remotely).
    pub editable: EditableXrayConfig,
}

/// Validates, mutates, conflict-checks, backs up, and writes policy settings.
pub async fn run_update_policy_settings<B>(
    backend: &B,
...
) -> PolicySettingsMutationOutcome
where
    B: SshBackend,
    B::Session: Sync,
{
    info!(target: "app", "Xray policy settings edit started");

    let mut editable = editable;
    let outcome = match update_policy_settings(&mut editable, request) {
        Ok(value) => value,
        Err(error) => {
            warn!(
                target: "app",
            ...
            return PolicySettingsMutationOutcome {
                result: Err(error),
            };
        }
    };
    ...
    info!(
        ...
        "policy settings mutation connect"
    );
    ...
    if let Err(error) = session.disconnect().await {
        warn!(
            ...
            "policy settings disconnect warning"
        );
    }

    match write_result {
        Ok(()) => {
            info!(target: "app", "Xray policy settings updated");
            PolicySettingsMutationOutcome {
                result: Ok(PolicySettingsMutationSuccess { editable }),
            }
        }
        Err(error) => PolicySettingsMutationOutcome {
            result: Err(error),
        },
    }
}
