//! Asynchronous Xray `stats` top-level settings modification orchestration (Roadmap §2.1:52).
//!
//! Mirrors [`super::dns_settings_ops`] — same validate → mutate → conflict-check → backup →
//! write pipeline, just targeting `stats` instead of `dns` (and, unlike `dns`, potentially
//! *removing* the section — see [`crate::xray::config::editable::EditableXrayConfig::with_stats_mut`]).
...
use crate::xray::{
    ...
    UpdateStatsSettingsRequest, update_stats_settings,
};

/// Outcome delivered from the stats-settings worker thread.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct StatsSettingsMutationOutcome {
    /// Success payload or classified error.
    pub result: Result<StatsSettingsMutationSuccess, ConfigModifyError>,
}

/// Successful remote write of updated stats settings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct StatsSettingsMutationSuccess {
    /// Editable model after the in-memory mutation (already applied remotely).
    pub editable: EditableXrayConfig,
}

/// Validates, mutates, conflict-checks, backs up, and writes stats settings.
pub async fn run_update_stats_settings<B>(
    backend: &B,
...
) -> StatsSettingsMutationOutcome
where
    B: SshBackend,
    B::Session: Sync,
{
    info!(target: "app", "Xray stats settings edit started");

    let mut editable = editable;
    let outcome = match update_stats_settings(&mut editable, request) {
        Ok(value) => value,
        Err(error) => {
            warn!(
                target: "app",
            ...
            return StatsSettingsMutationOutcome {
                result: Err(error),
            };
        }
    };
    ...
    info!(
        ...
        "stats settings mutation connect"
    );
    ...
    if let Err(error) = session.disconnect().await {
        warn!(
            ...
            "stats settings disconnect warning"
        );
    }

    match write_result {
        Ok(()) => {
            info!(target: "app", "Xray stats settings updated");
            StatsSettingsMutationOutcome {
                result: Ok(StatsSettingsMutationSuccess { editable }),
            }
        }
        Err(error) => StatsSettingsMutationOutcome {
            result: Err(error),
        },
    }
}
