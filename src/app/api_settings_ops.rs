//! Asynchronous Xray `api` top-level settings modification orchestration (Roadmap §2.1:54).
//!
//! Mirrors [`super::log_settings_ops`] — same validate → mutate → conflict-check → backup →
//! write pipeline, just targeting `api` instead of `log`.
...
use crate::xray::{
    ...
    UpdateApiSettingsRequest, update_api_settings,
};

/// Outcome delivered from the api-settings worker thread.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ApiSettingsMutationOutcome {
    /// Success payload or classified error.
    pub result: Result<ApiSettingsMutationSuccess, ConfigModifyError>,
}

/// Successful remote write of updated api settings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ApiSettingsMutationSuccess {
    /// Editable model after the in-memory mutation (already applied remotely).
    pub editable: EditableXrayConfig,
}

/// Validates, mutates, conflict-checks, backs up, and writes api settings.
pub async fn run_update_api_settings<B>(
    backend: &B,
...
) -> ApiSettingsMutationOutcome
where
    B: SshBackend,
    B::Session: Sync,
{
    info!(target: "app", "Xray api settings edit started");

    let mut editable = editable;
    let outcome = match update_api_settings(&mut editable, request) {
        Ok(value) => value,
        Err(error) => {
            warn!(
                target: "app",
            ...
            return ApiSettingsMutationOutcome {
                result: Err(error),
            };
        }
    };
    ...
    if let Err(error) = session.disconnect().await {
        warn!(
            ...
            "api settings disconnect warning"
        );
    }

    match write_result {
        Ok(()) => {
            info!(target: "app", "Xray api settings updated");
            ApiSettingsMutationOutcome {
                result: Ok(ApiSettingsMutationSuccess { editable }),
            }
        }
        Err(error) => ApiSettingsMutationOutcome {
            result: Err(error),
        },
    }
}
