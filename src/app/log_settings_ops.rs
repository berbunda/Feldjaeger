//! Asynchronous Xray log-settings modification orchestration.
...
use crate::xray::{
    ...
    UpdateLogSettingsRequest, update_log_settings,
};

/// Outcome delivered from the log-settings worker thread.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LogSettingsMutationOutcome {
    /// Success payload or classified error.
    pub result: Result<LogSettingsMutationSuccess, ConfigModifyError>,
}

/// Successful remote write of updated log settings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct LogSettingsMutationSuccess {
    /// Editable model after the in-memory mutation (already applied remotely).
    pub editable: EditableXrayConfig,
}

/// Validates, mutates, conflict-checks, backs up, and writes log settings.
pub async fn run_update_log_settings<B>(
    backend: &B,
...
) -> LogSettingsMutationOutcome
where
    B: SshBackend,
    B::Session: Sync,
{
    info!(target: "app", "Xray log settings edit started");

    let mut editable = editable;
    let outcome = match update_log_settings(&mut editable, request) {
        Ok(value) => value,
        Err(error) => {
            warn!(
                target: "app",
            ...
            return LogSettingsMutationOutcome {
                result: Err(error),
            };
        }
    };
    ...
    if let Err(error) = session.disconnect().await {
        warn!(
            ...
            "log settings disconnect warning"
        );
    }

    match write_result {
        Ok(()) => {
            info!(target: "app", "Xray log settings updated");
            LogSettingsMutationOutcome {
                result: Ok(LogSettingsMutationSuccess { editable }),
            }
        }
        Err(error) => LogSettingsMutationOutcome {
            result: Err(error),
        },
    }
}
