//! Asynchronous Xray `fakedns` top-level settings modification orchestration (Roadmap §2.1:47).
//!
//! Mirrors [`super::dns_settings_ops`] — same validate → mutate → conflict-check → backup →
//! write pipeline, just targeting `fakedns` instead of `dns`.
...
use crate::xray::{
    ...
    UpdateFakeDnsSettingsRequest, update_fakedns_settings,
};

/// Outcome delivered from the fakedns-settings worker thread.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FakeDnsSettingsMutationOutcome {
    /// Success payload or classified error.
    pub result: Result<FakeDnsSettingsMutationSuccess, ConfigModifyError>,
}

/// Successful remote write of updated fakedns settings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct FakeDnsSettingsMutationSuccess {
    /// Editable model after the in-memory mutation (already applied remotely).
    pub editable: EditableXrayConfig,
}

/// Validates, mutates, conflict-checks, backs up, and writes fakedns settings.
pub async fn run_update_fakedns_settings<B>(
    backend: &B,
...
) -> FakeDnsSettingsMutationOutcome
where
    B: SshBackend,
    B::Session: Sync,
{
    info!(target: "app", "Xray fakedns settings edit started");

    let mut editable = editable;
    let outcome = match update_fakedns_settings(&mut editable, request) {
        Ok(value) => value,
        Err(error) => {
            warn!(
                target: "app",
            ...
            return FakeDnsSettingsMutationOutcome {
                result: Err(error),
            };
        }
    };
    ...
    info!(
        ...
        "fakedns settings mutation connect"
    );
    ...
    if let Err(error) = session.disconnect().await {
        warn!(
            ...
            "fakedns settings disconnect warning"
        );
    }

    match write_result {
        Ok(()) => {
            info!(target: "app", "Xray fakedns settings updated");
            FakeDnsSettingsMutationOutcome {
                result: Ok(FakeDnsSettingsMutationSuccess { editable }),
            }
        }
        Err(error) => FakeDnsSettingsMutationOutcome {
            result: Err(error),
        },
    }
}
