//! Asynchronous Xray `env` top-level settings modification orchestration (Roadmap §2.1:55).
//!
//! Mirrors [`super::dns_settings_ops`] — same validate → mutate → conflict-check → backup →
//! write pipeline, just targeting `env` instead of `dns`.
...
use crate::xray::{
    ...
    UpdateEnvSettingsRequest, update_env_settings,
};

/// Outcome delivered from the env-settings worker thread.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct EnvSettingsMutationOutcome {
    /// Success payload or classified error.
    pub result: Result<EnvSettingsMutationSuccess, ConfigModifyError>,
}

/// Successful remote write of updated env settings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct EnvSettingsMutationSuccess {
    /// Editable model after the in-memory mutation (already applied remotely).
    pub editable: EditableXrayConfig,
}

/// Validates, mutates, conflict-checks, backs up, and writes env settings.
pub async fn run_update_env_settings<B>(
    backend: &B,
...
) -> EnvSettingsMutationOutcome
where
    B: SshBackend,
    B::Session: Sync,
{
    info!(target: "app", "Xray env settings edit started");

    let mut editable = editable;
    let outcome = match update_env_settings(&mut editable, request) {
        Ok(value) => value,
        Err(error) => {
            warn!(
                target: "app",
            ...
            return EnvSettingsMutationOutcome {
                result: Err(error),
            };
        }
    };
    ...
    if let Err(error) = session.disconnect().await {
        warn!(
            ...
            "env settings disconnect warning"
        );
    }

    match write_result {
        Ok(()) => {
            info!(target: "app", "Xray env settings updated");
            EnvSettingsMutationOutcome {
                result: Ok(EnvSettingsMutationSuccess { editable }),
            }
        }
        Err(error) => EnvSettingsMutationOutcome {
            result: Err(error),
        },
    }
}
