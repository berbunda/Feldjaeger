//! Asynchronous Xray `geodata` top-level settings modification orchestration (Roadmap §2.1:57).
//!
//! Mirrors [`super::version_settings_ops`] — same validate → mutate → conflict-check → backup →
//! write pipeline, just targeting `geodata` instead of `version`.
...
use crate::xray::{
    ...
    UpdateGeodataSettingsRequest, update_geodata_settings,
};

/// Outcome delivered from the geodata-settings worker thread.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct GeodataSettingsMutationOutcome {
    /// Success payload or classified error.
    pub result: Result<GeodataSettingsMutationSuccess, ConfigModifyError>,
}

/// Successful remote write of updated geodata settings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct GeodataSettingsMutationSuccess {
    /// Editable model after the in-memory mutation (already applied remotely).
    pub editable: EditableXrayConfig,
}

/// Validates, mutates, conflict-checks, backs up, and writes geodata settings.
pub async fn run_update_geodata_settings<B>(
    backend: &B,
...
) -> GeodataSettingsMutationOutcome
where
    B: SshBackend,
    B::Session: Sync,
{
    info!(target: "app", "Xray geodata settings edit started");

    let mut editable = editable;
    let outcome = match update_geodata_settings(&mut editable, request) {
        Ok(value) => value,
        Err(error) => {
            warn!(
                target: "app",
            ...
            return GeodataSettingsMutationOutcome {
                result: Err(error),
            };
        }
    };
    ...
    info!(
        ...
        "geodata settings mutation connect"
    );
    ...
    if let Err(error) = session.disconnect().await {
        warn!(
            ...
            "geodata settings disconnect warning"
        );
    }

    match write_result {
        Ok(()) => {
            info!(target: "app", "Xray geodata settings updated");
            GeodataSettingsMutationOutcome {
                result: Ok(GeodataSettingsMutationSuccess { editable }),
            }
        }
        Err(error) => GeodataSettingsMutationOutcome {
            result: Err(error),
        },
    }
}
