//! Asynchronous Xray `metrics` top-level settings modification orchestration (Roadmap §2.1:53).
//!
//! Mirrors [`super::dns_settings_ops`] — same validate → mutate → conflict-check → backup →
//! write pipeline, just targeting `metrics` instead of `dns`.
...
use crate::xray::{
    ...
    UpdateMetricsSettingsRequest, update_metrics_settings,
};

/// Outcome delivered from the metrics-settings worker thread.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MetricsSettingsMutationOutcome {
    /// Success payload or classified error.
    pub result: Result<MetricsSettingsMutationSuccess, ConfigModifyError>,
}

/// Successful remote write of updated metrics settings.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MetricsSettingsMutationSuccess {
    /// Editable model after the in-memory mutation (already applied remotely).
    pub editable: EditableXrayConfig,
}

/// Validates, mutates, conflict-checks, backs up, and writes metrics settings.
pub async fn run_update_metrics_settings<B>(
    backend: &B,
...
) -> MetricsSettingsMutationOutcome
where
    B: SshBackend,
    B::Session: Sync,
{
    info!(target: "app", "Xray metrics settings edit started");

    let mut editable = editable;
    let outcome = match update_metrics_settings(&mut editable, request) {
        Ok(value) => value,
        Err(error) => {
            warn!(
                target: "app",
            ...
            return MetricsSettingsMutationOutcome {
                result: Err(error),
            };
        }
    };
    ...
    info!(
        ...
        "metrics settings mutation connect"
    );
    ...
    if let Err(error) = session.disconnect().await {
        warn!(
            ...
            "metrics settings disconnect warning"
        );
    }

    match write_result {
        Ok(()) => {
            info!(target: "app", "Xray metrics settings updated");
            MetricsSettingsMutationOutcome {
                result: Ok(MetricsSettingsMutationSuccess { editable }),
            }
        }
        Err(error) => MetricsSettingsMutationOutcome {
            result: Err(error),
        },
    }
}
